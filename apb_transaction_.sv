...
logic pslverr;
constraint x{{psel,penable,pwrite} inside {[0:7]};}
virtual function apb_transaction_ copy();
copy=new();
...
class apb_write extends apb_transaction_;
...
class apb_read extends apb_transaction_;
...
class apb_write_low_high extends apb_transaction_;
...
class apb_read_low_high extends apb_transaction_;
...
class apb_read_out_of_range extends apb_transaction_;
constraint read_out{(pwrite==0)&&(paddr>(`DATA_DEPTH-1));}
virtual function apb_transaction_ copy();
...
class apb_write_out_of_range extends apb_transaction_;
constraint write_out{(pwrite==1)&&(paddr>(`DATA_DEPTH-1));}
virtual function apb_transaction_ copy();
...
class strobe_all_one extends apb_transaction_;
...
class strobe_all_zero extends apb_transaction_;
constraint all_zero{pstrb=='0;}
virtual function apb_transaction_ copy();
